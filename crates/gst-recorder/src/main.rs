...
fn main() {
    ...
    match server {
        Ok(server) => {
            ...
            let room_manager: Rooms = Arc::new(Mutex::new(RoomManager::new()));

            let app_config = config.clone();
            let tx_for_app = tx.clone();
            let rm_for_app = room_manager.clone();

            let xmpp_handle = thread::spawn(move || {
                match App::connect(&app_config, rm_for_app, tx_for_app, rx) {
            ...
            for request in server.incoming_requests() {
                ...
                thread::spawn(move || {
                    ...
                    match App::handle_join_room(&room_manager, &tx, &config.webrtc, &room) {
                        Ok(room_name) => {
