...
impl ParticipantPresence {
    ...
    pub fn from_presence(stanza: &Stanza) -> Option<PresenceLifecycle> {
        ...
        let e2ee_enabled_stanza = find_first(Some(&stanza), "jitsi_participant_e2ee.enabled");
        ...
        if presence_stanza.kind.is_empty() {
            ...
            if let Some(e2ee_enabled_stanza) = e2ee_enabled_stanza {
                let e2ee_stanza_text = e2ee_enabled_stanza.text().unwrap_or("false".to_string());
                if e2ee_stanza_text == "true".to_string() {
                    let ed25519 =
                        find_first(Some(&stanza), "jitsi_participant_e2ee.idkey.ed25519")?.text();
                    let curve25519 =
                        find_first(Some(&stanza), "jitsi_participant_e2ee.idkey.curve25519")?
                            .text();
                    participant.e2ee_enabled = true;
                    participant.curve25519 = curve25519;
                    participant.ed25519 = ed25519;
                }
            }
